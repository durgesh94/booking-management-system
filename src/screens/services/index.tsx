...
export default function ServicesScreen() {
  ...
  const getData = (isLoadMore: boolean) => {
    ...
    const pendingList: any = data.filter((item: any) => item.status === "ACTIVE");
  ...
  return (
    <View style={styles.container}>
      <View style={styles.view}>
        <Text style={styles.title}>Active Request List:</Text>
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item, index }: any) => <ItemView item={item} index={index} status={STATUS.ACTIVE} setItem={(item) => setItem(item)} setModalVisible={(flag) => setModalVisible(flag)} />}
